...
void AStarRailCharacter::MulticastElim_Implementation()
{
...
void AStarRailCharacter::Destroyed()
{
	...
	AStarRailGameMode* StarRailGameMode = Cast<AStarRailGameMode>(UGameplayStatics::GetGameMode(this));
...
void AStarRailCharacter::ReceiveDamage(AActor* DamagedActor, float Damage, const UDamageType* DamageType, AController* InstigatorController, AActor* DamageCauser)
{
	...
	if (Health == 0.f)
	{
		AStarRailGameMode* StarRailGameMode = GetWorld()->GetAuthGameMode<AStarRailGameMode>();
		if (StarRailGameMode)
		{
...
void AStarRailCharacter::OnRep_Health()
{
	UpdateHUDHealth();
	PlayHitReactMontage();
}
